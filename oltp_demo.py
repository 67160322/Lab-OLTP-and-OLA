...
with sqlite3.connect(p) as con:
    ...
    # Guarded UPDATE: เปลี่ยนสถานะเฉพาะเมื่อ order_id ตรงและ status ยังเป็น PENDING
    cur = con.execute(
...
# รอบแรก: status เป็น PENDING → UPDATE สำเร็จ rowcount=1
# รอบสอง: status เป็น PAID แล้ว → WHERE status='PENDING' ไม่ตรง → rowcount=0
# นี่คือ Idempotent Guard ของ OLTP เพื่อป้องกันการ update ซ้ำ
